...
feature_set = np.array(X_train).T
one_hot_labels = np.array(y_train).T

sc = StandardScaler()
sc.fit(feature_set)
feature_set = sc.transform(feature_set)

params = L_layer_model(feature_set, one_hot_labels, nn_architecture, print_cost=True, num_iterations=100)
print(L_model_forward(test_x.T, params, nn_architecture))