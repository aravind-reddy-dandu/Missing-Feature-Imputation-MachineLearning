...
class RandomForest():
    def __init__(self, x, y, n_trees, n_features, sample_sz, depth=10, min_leaf=5):
        ...
        self.trees = [self.create_tree() for i in tqdm(range(n_trees))]
...
rf = RandomForest(X_train, y_train, 1, 'sqrt', X_train.shape[0])
y_pred = rf.predict(np.array(test_x))

# x2_pred = pd.read_csv('D:\\Study\\ML\\Final_Project\\Sources\\regressionModels\\x2_rf_pred.csv')
x2_pred = np.array(y_pred)
test_y = np.array(test_y)
score = 1 - (((test_y - x2_pred) ** 2).sum() / ((test_y - test_y.mean()) ** 2).sum())
print(score)
